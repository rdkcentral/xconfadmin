...
import (
	...
	ev "github.com/rdkcentral/xconfwebconfig/rulesengine"
...
func TestRuleEval(t *testing.T) {
	t.Skip()
	if server == nil {
		panic("server is not init")
	}

	// setup data
	server.SetXconfData(core.TableFirmwareConfig, FirmwareConfigId1, firmwareConfig1Bytes, 3600)
	server.SetXconfData(core.TableFirmwareConfig, FirmwareConfigId2, firmwareConfig2Bytes, 3600)
	server.SetXconfData(core.TableFirmwareConfig, FirmwareConfigId3, firmwareConfig3Bytes, 3600)

	server.SetXconfData(core.TableFirmwareRule, firmwareRuleId1, firmwareRule1Bytes, 3600)
	server.SetXconfData(core.TableFirmwareRule, firmwareRuleId2, firmwareRule2Bytes, 3600)
	server.SetXconfData(core.TableFirmwareRule, firmwareRuleId3, firmwareRule3Bytes, 3600)

	macs := []string{mac3, "AA:AA:AA:BB:BB:BB", "AA:AA:AA:BB:BB:CC"}
	newList := core.NewGenericNamespacedList(namespaceListKey, core.MacList, macs)
	err := core.CreateGenericNamedListOneDB(newList)
	assert.NilError(t, err)

	// load data
	ruleBytes := server.GetAllXconfDataAsList(core.TableFirmwareRule, 0)
	processor := ev.NewRuleProcessorFactory().RuleProcessor()
