...
func TestFindMatchedRules(t *testing.T) {
	...
	for _, template := range templates {
		...
		} else if ruleType == "TEST_FW_ENV_MODEL_RULE" {
...
func TestEval(t *testing.T) {
	...
	templates, errdb := corefw.GetFirmwareRuleTemplateAllAsListDBForAS(corefw.DEFINE_PROPERTIES_TEMPLATE)
	assert.NilError(t, errdb)
	assert.Assert(t, templates != nil)

	templates, errdb = corefw.GetFirmwareRuleTemplateAllAsListDBForAS(corefw.BLOCKING_FILTER_TEMPLATE)
	assert.NilError(t, errdb)
	assert.Assert(t, templates != nil)
	assert.Equal(t, len(templates), 1)

	templates, errdb = corefw.GetFirmwareRuleTemplateAllAsListDBForAS(corefw.RULE_TEMPLATE)
	assert.NilError(t, errdb)
	assert.Assert(t, templates != nil)
