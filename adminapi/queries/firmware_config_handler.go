...
func PutPostFirmwareConfigEntitiesHandler(w http.ResponseWriter, r *http.Request, isPut bool) {
	...
	for i, entity := range entities {
		_, err := estbfirmware.GetFirmwareConfigOneDB(tenantId, entity.ID)
		if isPut && err != nil {
		...
		entity := entity
		if isPut {
			err2 = UpdateFirmwareConfigAS(tenantId, &entity, appType, false)
		} else {
			err2 = CreateFirmwareConfigAS(tenantId, &entity, appType, false)
		}
...
func PostFirmwareConfigFilteredHandler(w http.ResponseWriter, r *http.Request) {
	...
	filterContext[xcommon.TENANT_ID] = xhttp.GetTenantId(r.Context(), r)
