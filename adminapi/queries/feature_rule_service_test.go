...
func TestAddNewFeatureRuleAndReorganize(t *testing.T) {
	...
	t.Run("AddAtEnd", func(t *testing.T) {
		...
		assert.Equal(t, 1, len(result)) // Returns only the new item as altered sublist
	})
...
func TestValidateFeatureRule(t *testing.T) {
	...
	t.Run("ApplicationTypeMismatchWithParam", func(t *testing.T) {
		rdkFeature := makeFeatureForService("RdkCloudFeature", "rdkcloud")
		fr := &xwrfc.FeatureRule{
			Id:              uuid.New().String(),
			Name:            "TestRule",
...
func TestUpdateFeatureRule(t *testing.T) {
	...
	t.Run("ChangeApplicationType", func(t *testing.T) {
		// Create a feature with rdkcloud type for this test
		fRdkCloud := makeFeatureForService("RdkCloudFeatureForUpdate", "rdkcloud")

		fr := xwrfc.FeatureRule{
			Id:              existingRule.Id,
			Name:            existingRule.Name,
			ApplicationType: "rdkcloud",
			FeatureIds:      []string{fRdkCloud.ID},
			Priority:        1,
			Rule:            makeRuleForService(),
		}
		...
		// Cleanup
		DeleteOneFromDao(db.TABLE_FEATURES, fRdkCloud.ID)
	})
	...
	t.Run("UpdateWithDifferentPriority", func(t *testing.T) {
		...
		SetOneInDao(db.TABLE_FEATURE_CONTROL_RULES, existingRule.Id, existingRule)
		...
		SetOneInDao(db.TABLE_FEATURE_CONTROL_RULES, fr2.Id, fr2)
		...
		// Cleanup
		DeleteOneFromDao(db.TABLE_FEATURE_CONTROL_RULES, fr2.Id)
	})
...
func TestUpdateFeatureRuleByPriorityAndReorganize(t *testing.T) {
	...
	t.Run("AddNewItemToEmptyList", func(t *testing.T) {
		...
		assert.NotNil(t, result)
		assert.Equal(t, 1, len(result))
		assert.Equal(t, newItem.Id, result[0].Id)
	})
...
func TestImportOrUpdateAllFeatureRule(t *testing.T) {
	...
	t.Run("ImportNewRules", func(t *testing.T) {
		newRule1 := xwrfc.FeatureRule{
			...
			Rule: &re.Rule{
				Condition: CreateCondition(
					*re.NewFreeArg(re.StandardFreeArgTypeString, "env"),
					re.StandardOperationIs,
					"QA",
				),
			},
...
func TestChangeFeatureRulePriorities(t *testing.T) {
	...
	t.Run("ChangePriority_MoveUp", func(t *testing.T) {
		...
		fr2.Priority = 2
		fr3.Priority = 3
		SetOneInDao(db.TABLE_FEATURE_CONTROL_RULES, fr1.Id, fr1)
		SetOneInDao(db.TABLE_FEATURE_CONTROL_RULES, fr2.Id, fr2)
		SetOneInDao(db.TABLE_FEATURE_CONTROL_RULES, fr3.Id, fr3)
