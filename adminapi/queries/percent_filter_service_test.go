...
func TestUpdatePercentFilter_AppTypeAndGlobalRangeValidation(t *testing.T) {
	...
	w2 := newWrapper(-1)
	assert.Equal(t, 400, UpdatePercentFilter(db.GetDefaultTenantId(), "stb", w2).Status)
	w3 := newWrapper(101)
	assert.Equal(t, 400, UpdatePercentFilter(db.GetDefaultTenantId(), "stb", w3).Status)
}
...
func TestUpdatePercentFilter_WhitelistMismatch(t *testing.T) {
	...
	assert.Equal(t, 400, UpdatePercentFilter(db.GetDefaultTenantId(), "stb", w).Status)
}
...
func TestUpdatePercentFilter_EnvModelPercentageValidation(t *testing.T) {
	...
	assert.Equal(t, 400, UpdatePercentFilter(db.GetDefaultTenantId(), "stb", w4).Status)
	...
	assert.Equal(t, 400, UpdatePercentFilter(db.GetDefaultTenantId(), "stb", w5).Status)
}
...
func TestConvertPercentageBean_SumAndWhitelist(t *testing.T) {
	...
	pct := convertPercentageBean(db.GetDefaultTenantId(), bean)
	assert.NotNil(t, pct)
