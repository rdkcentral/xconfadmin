...
func TestPostFirmwareRuleHandler_TagRuleTemplateValidation(t *testing.T) {
	...
	t.Run("compound multiple EXISTS conditions accepted", func(t *testing.T) {
		status, body := postFirmwareRule(t, `{
			"name": "TagRuleCompoundMultipleExists",
			"applicationType": "stb",
			"type": "TAG_RULE",
			"active": true,
			"rule": {
				...
				"compoundParts": [
					...
					},
					{
						...
						"relation": "AND",
						"condition": {
							"freeArg": {"type": "STRING", "name": "another:test:tag"},
							"operation": "EXISTS"
						}
					}
				]
	...
	t.Run("model-only condition without EXISTS rejected", func(t *testing.T) {
		...
		assert.Assert(t, strings.Contains(body, "requires at least one EXISTS condition"),
			"expected EXISTS requirement error in body, got: "+body)
	})
}
