...
func TestUpdateLocationFilter_ValidationFailures(t *testing.T) {
	...
	assert.Equal(t, 400, UpdateLocationFilter(db.GetDefaultTenantId(), "", lfAppInvalid).Status)
}
...
func TestUpdateLocationFilter_MissingConditionsBranches(t *testing.T) {
	...
	assert.Equal(t, 400, UpdateLocationFilter(db.GetDefaultTenantId(), "stb", lf2).Status)
...
func TestUpdateLocationFilter_SuccessAndDelete(t *testing.T) {
	...
	common.DeleteOneEnvironment(db.GetDefaultTenantId(), "E2")
	...
	resp := UpdateLocationFilter(db.GetDefaultTenantId(), "stb", lf)
	if resp.Status != 200 {
		t.Fatalf("expected 200 got %d, error: %v", resp.Status, resp.Error)
	}
	assert.NotEmpty(t, lf.Id)
	// delete existing
	delResp := DeleteLocationFilter(db.GetDefaultTenantId(), "LFSUCC", "stb")
	assert.Equal(t, 204, delResp.Status, "First delete should return 204, error: %v", delResp.Error)
	// delete again (noop)
	delResp2 := DeleteLocationFilter(db.GetDefaultTenantId(), "LFSUCC", "stb")
