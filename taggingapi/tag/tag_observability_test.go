...
func TestAddMembersHandler_RequestEndsAuditFields(t *testing.T) {
	...
	assert.Equal(t, db.GetDefaultTenantId(), fields["tenant"])
...
func TestAddMembers_XdasOutageDoesNotFloodLogs(t *testing.T) {
	...
	stats, err := AddMembersWithXdas(db.GetDefaultTenantId(), "outage-tag", members, "")
...
func TestDeleteTag_LogsCarryAuditId(t *testing.T) {
	...
	err := DeleteTag(db.GetDefaultTenantId(), "audited-tag", "audit-123")
	assert.NoError(t, err)
...
func TestAddMembers_CassandraOutageDoesNotFloodLogs(t *testing.T) {
	...
	stats, err := AddMembersWithXdas(db.GetDefaultTenantId(), "cass-outage-tag", members, "")
	assert.Error(t, err)
...
// Reverse lookup: XDAS tag count before tenant filtering and the filtered
// result count are both visible.
func TestGetTagsByMemberHandler_AuditFields(t *testing.T) {
	...
	// before setting result counts — but op/tenant must be set regardless.
	assert.Equal(t, db.GetDefaultTenantId(), fields["tenant"])
}
