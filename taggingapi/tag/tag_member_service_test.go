...
func TestBatchSizeValidation(t *testing.T) {
	...
	_, _, err := AddMembers(db.GetDefaultTenantId(), "test-tag", []string{}, "")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "member list is empty")

	_, _, err = RemoveMembers(db.GetDefaultTenantId(), "test-tag", []string{})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "member list is empty")
	...
	_, _, err = AddMembers(db.GetDefaultTenantId(), "test-tag", largeMembers, "")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "batch size")
	assert.Contains(t, err.Error(), "exceeds maximum")

	_, _, err = RemoveMembers(db.GetDefaultTenantId(), "test-tag", largeMembers)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "batch size")
	assert.Contains(t, err.Error(), "exceeds maximum")
}
