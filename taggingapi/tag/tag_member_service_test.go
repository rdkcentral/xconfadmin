...
func TestAddMembersV2Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	// This would require proper database setup
	// tagId := "test-tag-integration"
	// members := []string{
	//     "00:11:22:33:44:55",
	//     "AA:BB:CC:DD:EE:FF",
	//     "12:34:56:78:90:AB",
	// }
	// err := AddMembersV2(tagId, members)
	// assert.NoError(t, err)

	// Verify members were added
	// response, err := GetMembersV2Paginated(tagId, 10, "")
	// assert.NoError(t, err)
	// assert.Equal(t, len(members), len(response.Data))

	t.Log("Integration test placeholder - requires database setup")
}

func TestRemoveMembersV2Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
...
// V2 XDAS Integration Tests
func TestAddMembersV2WithXDASValidation(t *testing.T) {
	// Test input validation without requiring XDAS/database setup

	// Test empty members list
	err := AddMembersWithXdas("test-tag", []string{})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "member list is empty")
	...
	err = AddMembersWithXdas("test-tag", largeMembers)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "batch size")
	assert.Contains(t, err.Error(), "exceeds maximum")
}

func TestRemoveMembersV2WithXDASValidation(t *testing.T) {
	// Test input validation without requiring XDAS/database setup

	// Test empty members list
	err := RemoveMembersV2WithXdas("test-tag", []string{})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "member list is empty")
	...
	err = RemoveMembersV2WithXdas("test-tag", largeMembers)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "batch size")
	assert.Contains(t, err.Error(), "exceeds maximum")
}

func TestRemoveMemberV2WithXDASValidation(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping XDAS integration test in short mode")
	}

	// Test single member removal validation
	// This will fail due to missing XDAS setup, but we can test the API
	err := RemoveMemberV2WithXdas("test-tag", "test-member")
	// We expect this to fail due to missing XDAS setup, but it shouldn't panic
	assert.Error(t, err)
	t.Logf("Expected error due to missing XDAS setup: %v", err)
}
