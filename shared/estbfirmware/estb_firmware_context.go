...
func GetContextConverted(ctx map[string]string) *ConvertedContext {
	...
	// c.RawTime = c.getContextValue(core.TIME)
	// tm := c.GetTime()
...
func (c *ConvertedContext) CreateCapabilitiesList() []Capabilities {
	...
	capabilities := c.GetCapabilities()
	for i, strCap := range capabilities {
		cap := strings.ToUpper(strings.TrimSpace(strCap))
		if util.IsBlank(cap) && i+1 == len(capabilities) {
			continue
		}
		switch cap {
		case "RCDL":
		...
		default:
			log.Warn(fmt.Sprintf("Unknown capability '%s' will be ignored. Supplied Capabilities: %s", cap, c.getContextValue(core.CAPABILITIES)))
		}
	}
	return capList
}
...
func (c *ConvertedContext) SetEnvConverted(env string) {
	if env != "" {
		c.Env = core.NewEnvironment(env, "").ID
	}
}
...
func (c *ConvertedContext) SetModelConverted(model string) {
	if model != "" {
		c.Model = core.NewModel(model, "").ID
	}
}
...
func (c *ConvertedContext) GetTime() *time.Time {
	stime := c.getContextValue(core.TIME)
	if stime != "" {
...
func (c *ConvertedContext) SetTime(t time.Time) {
	c.setContextValue(core.TIME, t.Format(DATE_TIME_SEC_FORMATTER))
}

func (c *ConvertedContext) GetIpAddress() string {
	return c.getContextValue(core.IP_ADDRESS)
}

func (c *ConvertedContext) SetIpAddress(ipAddress string) {
	c.setContextValue(core.IP_ADDRESS, ipAddress)
}

func (c *ConvertedContext) GetBypassFilters() string {
	return c.getContextValue(core.BYPASS_FILTERS)
}

func (c *ConvertedContext) SetBypassFilters(bypassFilters string) {
	c.setContextValue(core.BYPASS_FILTERS, bypassFilters)
}

func (c *ConvertedContext) GetForceFilters() string {
	return c.getContextValue(core.FORCE_FILTERS)
}

func (c *ConvertedContext) SetForceFilters(forceFilters string) {
	c.setContextValue(core.FORCE_FILTERS, forceFilters)
}
...
func (c *ConvertedContext) GetTimeZone() string {
	return c.getContextValue(core.TIME_ZONE)
}

func (c *ConvertedContext) SetTimeZone(tz string) {
	c.setContextValue(core.TIME_ZONE, tz)
}

func (c *ConvertedContext) GetTimeZoneOffset() string {
	return c.getContextValue(core.TIME_ZONE_OFFSET)
}

func (c *ConvertedContext) SetTimeZoneOffset(timeZoneOffset string) {
	c.setContextValue(core.TIME_ZONE_OFFSET, timeZoneOffset)
}

func (c *ConvertedContext) GetCapabilities() []string {
	strCapabilities := c.getContextValue(core.CAPABILITIES)
...
func (c *ConvertedContext) SetCapabilities(capabilities []string) {
	...
	c.setContextValue(core.CAPABILITIES, strCapabilities)
}

func (c *ConvertedContext) GetPartnerId() string {
	return c.getContextValue(core.PARTNER_ID)
}

func (c *ConvertedContext) SetPartnerId(partnerId string) {
	c.setContextValue(core.PARTNER_ID, partnerId)
}

func (c *ConvertedContext) GetAccountId() string {
	return c.getContextValue(core.ACCOUNT_ID)
}

func (c *ConvertedContext) SetAccountId(accountId string) {
	c.setContextValue(core.ACCOUNT_ID, accountId)
}
