...
func NewRuleInfo(filterOrRule interface{}) *RuleInfo {
	...
	case *corefw.FirmwareRule:
		isBlocking := false
		if t.ApplicableAction != nil && t.ApplicableAction.ActionType == corefw.BLOCKING_FILTER {
			isBlocking = true
		}
		return &RuleInfo{
			ID:       t.ID,
...
func GetConfigChangeLogsOnly(mac string) []*sharedef.ConfigChangeLog {
	...
	if err == nil {
		...
		for _, log := range data {
			configLog, ok := log.(*sharedef.ConfigChangeLog)
