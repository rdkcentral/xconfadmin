...
func (obj *FirmwareConfig) Validate() error {
	...
	err := core.ValidateApplicationType(obj.ApplicationType)
	if err != nil {
		return err
	}
...
func CreateFirmwareConfigFacadeResponse(firmwareConfigFacade FirmwareConfigFacade) FirmwareConfigFacadeResponse {
	...
	if _, ok := firmwareConfigResponse[core.REBOOT_IMMEDIATELY]; !ok {
		firmwareConfigResponse[core.REBOOT_IMMEDIATELY] = false
	}
...
func (fc *FirmwareConfig) ToPropertiesMap() map[string]interface{} {
	...
	util.PutIfValuePresent(dataMap, core.ID, fc.ID)
	util.PutIfValuePresent(dataMap, core.UPDATED, fc.Updated)
	util.PutIfValuePresent(dataMap, core.DESCRIPTION, fc.Description)
	util.PutIfValuePresent(dataMap, core.SUPPORTED_MODEL_IDS, fc.SupportedModelIds)
	util.PutIfValuePresent(dataMap, core.FIRMWARE_DOWNLOAD_PROTOCOL, fc.FirmwareDownloadProtocol)
	util.PutIfValuePresent(dataMap, core.FIRMWARE_FILENAME, fc.FirmwareFilename)
	util.PutIfValuePresent(dataMap, core.FIRMWARE_LOCATION, fc.FirmwareLocation)
	util.PutIfValuePresent(dataMap, core.FIRMWARE_VERSION, fc.FirmwareVersion)
	util.PutIfValuePresent(dataMap, core.IPV6_FIRMWARE_LOCATION, fc.Ipv6FirmwareLocation)
	util.PutIfValuePresent(dataMap, core.UPGRADE_DELAY, fc.UpgradeDelay)
	util.PutIfValuePresent(dataMap, core.REBOOT_IMMEDIATELY, fc.RebootImmediately)
	util.PutIfValuePresent(dataMap, core.MANDATORY_UPDATE, fc.MandatoryUpdate)
...
func (fcf *FirmwareConfigFacade) MarshalJSON() ([]byte, error) {
	...
	fields := []string{
		core.ID,
		core.DESCRIPTION,
		core.FIRMWARE_DOWNLOAD_PROTOCOL,
		core.FIRMWARE_FILENAME,
		core.FIRMWARE_LOCATION,
		core.FIRMWARE_VERSION,
		core.IPV6_FIRMWARE_LOCATION,
		core.UPGRADE_DELAY,
		core.REBOOT_IMMEDIATELY,
		core.SUPPORTED_MODEL_IDS,
		core.MANDATORY_UPDATE,
		core.UPDATED,
	}
...
func AddExpressionToIpRuleBean(ipRuleBean *sharedef.IpRuleBean) {
	expression := sharedef.Expression{}
...
func (ff *FirmwareConfigFacade) SetFirmwareLocation(location string) {
	ff.SetStringValue(core.FIRMWARE_LOCATION, location)
}

func (ff *FirmwareConfigFacade) GetFirmwareLocation() string {
	return ff.GetStringValue(core.FIRMWARE_LOCATION)
}

func (ff *FirmwareConfigFacade) GetFirmwareFilename() string {
	return ff.GetStringValue(core.FIRMWARE_FILENAME)
}

func (ff *FirmwareConfigFacade) GetFirmwareVersion() string {
	return ff.GetStringValue(core.FIRMWARE_VERSION)
}

func (ff *FirmwareConfigFacade) GetIpv6FirmwareLocation() string {
	return ff.GetStringValue(core.IPV6_FIRMWARE_LOCATION)
}

func (ff *FirmwareConfigFacade) GetUpgradeDelay() int {
	if val, ok := ff.Properties[core.UPGRADE_DELAY]; ok {
		if val == nil {
			return 0
		}
...
func GetFirmwareConfigAsMapDB(applicationType string) (configMap map[string]sharedef.FirmwareConfig, err error) {
	...
	configMap = make(map[string]sharedef.FirmwareConfig)

	for _, r := range rulelst {
		cfg, ok := r.(*sharedef.FirmwareConfig)
		if !ok {
			continue
		}
